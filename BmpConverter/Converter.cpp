...
void drawShape(const ImgInfo* info, HDC hdcWindow, HWND hwnd) {
    ...
    for (int x = 0; x < pictWidth; ++x) {
        for (int y = 0; y < pictHeight; ++y) {
            SetPixel(hdcWindow, windowWidth - pictWidth - x + windowWidth / 15, windowHeight / 2 + y - pictHeight / 2, pixelColors->at(pixelColors->size() - (1 + pictWidth * y + x)));
        }
    }
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
        ...
        string filename = "C:\\Users\\Евгений\\Downloads\\3.bmp";
